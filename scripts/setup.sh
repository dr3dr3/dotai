...
# ── Codex CLI (personal) ──────────────────────────────────────────────────────
#
# Team context for Codex is AGENTS.override.md, emitted by ai-devex into each
# RoE repo. This block is the *user* layer: ~/.codex/config.toml (byte cap +
# CLAUDE.md fallback), ~/.codex/AGENTS.md (personal context), and
# ~/.agents/skills (personal skills — Codex's user-scope skill root).

CODEX_HOME="${CODEX_HOME:-$HOME/.codex}"
CODEX_CONFIG="$CODEX_HOME/config.toml"
CODEX_AGENTS_MD="$CODEX_HOME/AGENTS.md"
CODEX_CONFIG_TEMPLATE="$DEVEX_DIR/templates/codex-config.toml"
CODEX_USER_SKILLS="$HOME/.agents/skills"

mkdir -p "$CODEX_HOME" "$CODEX_USER_SKILLS"
...
# Personal ~/.codex/AGENTS.md — same splice as ~/.claude/CLAUDE.md, so the
# team.md block survives ai-devex (which never touches CODEX_HOME).
if [ ! -f "$CODEX_AGENTS_MD" ]; then
  echo ""
  echo "→ Creating ~/.codex/AGENTS.md (user-level Codex context)"
  cat > "$CODEX_AGENTS_MD" << 'EOF'
# Global Codex Context

# Project context is AGENTS.override.md (generated by ai-devex) or AGENTS.md
# in the repo you launched from. This file is personal, all-projects.
EOF
  echo "  ✓ ~/.codex/AGENTS.md created"
fi

CODEX_BLOCK_BODY="$(mktemp)"
{
  echo "$DOTAI_BLOCK_START"
  echo ""
  echo "<!-- Generated by dotai/scripts/setup.sh from dotai/context/."
...
echo ""
echo "→ Syncing personal context block into ~/.codex/AGENTS.md"
if command -v python3 &>/dev/null; then
  python3 - "$CODEX_AGENTS_MD" "$CODEX_BLOCK_BODY" "$DOTAI_BLOCK_START" "$DOTAI_BLOCK_END" <<'SPLICEPY'
import sys
...
if start in current and end in current:
    ...
    sys.exit("  ✗ ~/.codex/AGENTS.md has a broken DOTAI-GENERATED block "
             "(one delimiter missing). Fix it by hand, then re-run.")
else:
    updated = current.rstrip("\n") + "\n\n" + block + "\n"
    action = "added"
...
rm -f "$CODEX_BLOCK_BODY"

echo ""
echo "→ Linking skills into ~/.agents/skills/ (Codex user scope)"
for d in "$DEVEX_DIR/skills/"*/; do
  [ -f "${d}SKILL.md" ] || continue
  name="$(basename "$d")"
  ln -sfn "${d%/}" "$CODEX_USER_SKILLS/$name"
  echo "  ✓ $name"
done
...
echo "  • Codex:                         codex login   then   codex"
echo ""
